...
export default function useMoveToTrash() {
	...
	const onAction = (file?: FileDataType) => {
		if (!can.fileActions.includes("Move To Trash")) return;
		...
		confirmDeletePrompt({
			onConfirm: () => {
				...
				toast.promise(
					axios.post(route(`${routes.routeNamePrefix}.files.delete`), data)
						.then(response => {
							router.reload({
								only: ["descendants"],
								preserveScroll: true
					...
					{
						loading: 'Deleting...',
						success: <b>files moved to trash successfully</b>,
